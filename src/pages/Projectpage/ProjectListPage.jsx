  ...
  const filteredProjects = projects.filter(
    (project) =>
      (project.project?.projectName || "")
        .normalize("NFC") 
        .toLowerCase()
        .includes(searchProject.normalize("NFC").toLowerCase()) ||
      ...
      <div className="flex flex-col min-h-screen p-8 w-[95%] mx-auto bg-gray-100">
        ...
        <table className="w-full rounded-lg shadow-lg border-spacing-0 border-separate">
          <thead className="bg-blue-100">
            <tr>
              <th className="p-6 text-center text-gray-700 font-semibold">
                No
              </th>
              <th className="p-6 text-center text-gray-700 font-semibold">
                Project
          ...
          <tbody>
            {currentProjects.map((project, index) => (
              <tr
                key={index}
                className="bg-white hover:bg-gray-50 transition-all duration-150 rounded-lg shadow-md my-2"
              >
                <td className="p-6 text-center text-gray-600">
                  {(page - 1) * itemsPerPage + index + 1}
                </td>
                <td className="p-6 text-center text-gray-800 font-medium">
                  {project.project?.projectName}
                </td>
                <td className="p-6 text-center text-gray-600">
                  {project.user?.displayName}
                ...
                <td className="p-6 text-center text-gray-600">
                  {" "}
                  {moment(project.project?.createdAt).format("LLL")}
                </td>
